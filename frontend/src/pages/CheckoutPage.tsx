...
export default function CheckoutPage() {
  ...
  const { data: orders, loading } = usePolling<Order[]>(fetchOrders, { enabled: Boolean(sessionId) });
  const billableOrders = useMemo(() => orders?.filter((order) => order.status !== 'CANCELLED') ?? [], [orders]);
  const total = useMemo(() => billableOrders.reduce((sum, order) => sum + order.total, 0), [billableOrders]);
      ...
      <div className="mx-auto max-w-2xl">
        ...
        {paid ? (
          ...
          <div className="space-y-6">
            <Card>
              <CardHeader><CardTitle>Bill summary</CardTitle><CardDescription>{loading ? 'Loading order rounds...' : `${billableOrders.length} billable order round${billableOrders.length === 1 ? '' : 's'}`}</CardDescription></CardHeader>
              <CardContent>
                <div className="space-y-3">
                  {billableOrders.map((order) => <div key={order.id} className="flex justify-between text-sm"><span>Order round #{order.id}</span><span>฿{order.total.toFixed(2)}</span></div>)}
                  {!loading && billableOrders.length === 0 ? <p className="text-sm text-muted-foreground">No billable orders found.</p> : null}
                  <div className="flex justify-between border-t border-border pt-4 text-xl font-bold"><span>Total</span><span>฿{total.toFixed(2)}</span></div>
                </div>
              </CardContent>
            </Card>
            <Card>
              <CardHeader><CardTitle>Payment method</CardTitle><CardDescription>Choose how this session will be paid.</CardDescription></CardHeader>
              <CardContent>
